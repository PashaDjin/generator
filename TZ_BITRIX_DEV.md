...
### 2.1 Используемые существующие поля (НЕ создавать!)

В сущности **"Сделка" (Deal)** уже есть:

| Поле | Откуда берется | Описание |
|------|----------------|----------|
| ID сделки | `ID` | Используется как UID (формат: BITRIX-12345) |
| Адрес объекта | `UF_CRM_ADDRESS` | Адрес объекта (уже существует) |
| Название сделки | `TITLE` | Имя клиента |
| Телефон контакта | `PHONE[0].VALUE` | Телефон из связанного контакта |

### 2.2 Новые поля для РЕЗУЛЬТАТА (создать обязательно!)

В сущности **"Сделка" (Deal)** создать 3 пользовательских поля для ВЫВОДА результата:

| Название | Символьный код | Тип | Обязательное | Описание |
|----------|----------------|-----|--------------|----------|
| UID сметы | `UF_CRM_SMETA_UID` | `string` | Нет | Уникальный идентификатор объекта (BITRIX-12345) |
| Ссылка на смету | `UF_CRM_SMETA_LINK` | `url` | Нет | URL файла Google Sheets (кликабельна) |
| Папка клиента | `UF_CRM_FOLDER_LINK` | `url` | Нет | URL папки в Google Drive (кликабельна) |

**Путь:** CRM → Настройки → Настройка полей → Сделка → Добавить поле

**ВАЖНО:** Эти поля нужны для того, чтобы GAS мог ВЕРНУТЬ результат - ссылки на созданные файлы. Без них менеджеры не увидят ссылки на сметы!
...
**Плейсхолдеры Битрикса (используют СУЩЕСТВУЮЩИЕ поля):**
- `{=Document:ID}` - ID сделки (стандартное поле)
- `{=Document:TITLE}` - Название сделки = Имя клиента (стандартное поле)
- `{=Document:UF_CRM_ADDRESS}` - Адрес объекта (существующее поле)
- `{=Document:PHONE[0].VALUE}` - Телефон из связанного контакта (стандартное поле)
...
| Поле | Символьный код | Обязательное | Где находится |
|------|----------------|--------------|---------------|
| Название сделки | `TITLE` | ✅ Да | Стандартное поле сделки |
| Адрес объекта | `UF_CRM_ADDRESS` | ✅ Да | Существующее пользовательское поле |
| Телефон | `PHONE[0].VALUE` | ✅ Да | Телефон из связанного контакта |

**Все эти поля уже должны существовать в вашем Битрикс24!**
